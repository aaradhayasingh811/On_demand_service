...
const BookingPro = React.memo(({ item }) => {
  ...
  const handleonClick = async(item , stat) =>{
    try {
      ...
      const response = await axios.patch(`${import.meta.env.HOST_URL}/api/v1/change-status`,Data);
      if(stat === "Success"){
        await axios.post(`${import.meta.env.HOST_URL}/api/v1/send-email`,sendData);
        console.log(sendData)
