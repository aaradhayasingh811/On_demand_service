...
const BookingPro = React.memo(({ item }) => {
  ...
  const handleonClick = async(item , stat) =>{
    try {
      ...
      const response = await axios.patch(`https://on-demand-service-m5nh.onrender.com/api/v1/change-status`,Data);
      if(stat === "Success"){
        await axios.post(`https://on-demand-service-m5nh.onrender.com/api/v1/send-email`,sendData);
        console.log(sendData)
